...
import 'package:app_tuoi_cay/widget/box_time_watering.dart';
...
class _StatusPageState extends State<StatusPage> {
  ...
  DatabaseReference  getdata_status = FirebaseDatabase.instance.ref();

  Call_Data() async {
    final ref = FirebaseDatabase.instance.ref();
    final snapshot = await ref.child('humidity_current').get();
    print('object');
    if (snapshot.exists) {
  ...
  String nowtime = '';
  Load_Time(){
    nowday = DateFormat('EEEE, dd MMMM').format(now);
    nowtime = DateFormat.Hm().format(now);
  }
  Body(){
    ...
    return StreamBuilder(
      stream: getdata_status.onValue,
        builder: (BuildContext context,AsyncSnapshot snapshot){
          Load_Time();
          if (snapshot.hasData){
            DataSnapshot dataValues = snapshot.data.snapshot;
            Map<dynamic, dynamic> data = dataValues.value as Map<dynamic, dynamic>;
    ...
    //     query: getdata_status,
    //     itemBuilder: (context, snapshot, animation, index){
    //       var humidity_current = snapshot.child('getdata_status');
    //       print('humidity_current :$humidity_current');
    //       return SingleChildScrollView(
    //         child: Padding(
    //           padding: EdgeInsets.symmetric(horizontal: widthR * 100),
    //           child: Column(
